...
- Die zentrale Sentinel-Erkennung bleibt für ungültige Integer-Grenzwerte aktiv; definierte Sonderzustände können punktbezogen behandelt werden.
- Regressionstests für Abtau-Sonderzustände und REST-basierte Enum-Auswertung ergänzt.
- README auf den aktuellen Stand von 0.9.8 gebracht.
...
- Bereits registrierte, automatisch erzeugte Entity-IDs werden beim Setup auf sprachneutrale IDs im Schema `<domain>.nibe_api_<translation_key>` migriert.
...
- Wiederholte Authentifizierungsfehler erzeugen die Persistent Notification und die zugehörige Host-/IP-Auflösung nur einmal pro zusammenhängender Störung.
...
- Schreibschutz für **Heizung zulassen** und **Kühlung zulassen** verschärft.
...
- Reauthentifizierungsdialog und Persistent Notifications für Verbindungs-/Authentifizierungsfehler verbessert.

## 0.6.0

- Fallback für nicht auswertbare `/points`-Antworten mit Backoff eingeführt.
